function MultiplLinearRegressionPlotConC( EXPbgdataScriptEGG,cepas,cepa,refs,colores,tfijo)%
...
for i = cepa %[cepas.refComp cepas.hap3]% 1:96
    ...
    for dia = 1:length(EXPbgdataScriptEGG.Tdays)-1
        ...
        t = EXPbgdataScriptEGG.tOut(start:finish)/24;
        RoC = LOGRoC(start:finish);
        T = EXPbgdataScriptEGG.Tdays(dia)/24;%este /24 no siempre se necesita!
    	...
    	hora=t*24;
         for k=1:length(t)
             ...
             plot([t(k)+T;t(k)+T], [yfit(k)';yfit(k)'+C(k)'], '-', 'color',[.52 .52 .52])
             hold on
         end
        plot(t+T, RoC,'.','color', colores(i,:) ,'linewidth',.5,'MarkerFaceColor', colores(i,:) );% ) [1/16*T 1/16*T 1/16*T]
        hold on
        plot([T  T+t],[ynoG yfit'],'-','color',colores(i,:)); %PlotConError(T+t,yfit,yfitTOPCI, yfitBOTCI,colores(i,:),'s-',3,.0051);%Misma Cosa, con o sin error
        plot(T+t,yfit,'x','color',colores(i,:),'MarkerSize',5,'markerfacecolor',colores(i,:)); %PlotConError(T+t,yfit,yfitTOPCI, yfitBOTCI,colores(i,:),'s-',3,.0051);%Misma Cosa, con o sin error
